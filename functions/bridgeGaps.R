...
bridgeGaps <- function(path, intersectionNodeA, intersectionNodeB, new.links) {
  ...
  if (length(breakpoints) > 1)  {
    for (i in 1:(length(breakpoints)-1)) {
      ...
      # length of section (difference between the pair of breakpoints - ie number of 1m points)
      sectionLength <- abs(breakpoints[i+1] - breakpoints[i])
      ...
      new.links <- rbind(new.links, newlink)
    }
  }
